...
import { RouteToCommit, SbInterceptor } from '../interfaces';
...
function compileRoute(routeInstructions: RouteToCommit) {
  const { key, instanceWrapper } = routeInstructions;
  const ctrlInterceptors: SbInterceptorMetadataForTarget
    = Reflect.getMetadata(SERVICE_BUS_INTERCEPTORS_METADATA, instanceWrapper.metatype.prototype);
