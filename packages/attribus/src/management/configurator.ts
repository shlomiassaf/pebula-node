...
export class SbConfigurator {
  private sbLogger: SbLogger = NoopLogger.shared;
  ...
  async verifyQueue(queueName: string, provision: SbEntityProvisionOption<SbQueueEntityProvision>, forceCreate = false): Promise<SbQueue> {
    ...
    if (forceCreate || this.shouldCreate(queue, provision.type)) {
      queue = await this.managementClient.upsertQueue(queueName, provision.params, !queue);
    }

    // Add to cache so we won't verify this one again.
    if (queue) {
  ...
  async verifyTopic(topicName: string, provision: SbEntityProvisionOption<SbTopicEntityProvision>, forceCreate = false): Promise<SbTopic> {
    // check if already verified for this connector...
    ...
    if (forceCreate || this.shouldCreate(topic, provision.type)) {
  ...
  async verifySubscription(topicName: string,
                           ...
                           forceCreate = false,): Promise<SbSubscription> {
    ...
    if (forceCreate || this.shouldCreate(subscription, provision.type)) {
